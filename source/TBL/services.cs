...
{
	/// <summary>
	/// Antwortcodes der Services über den <see cref="ServiceResponseLog">serviceResponseLog</see>
	/// </summary>
	public enum ServiceResponseCodes
	{
		/// <summary>
		/// Vorher erhaltenes Kommando wurde fehlerfrei ausgeführt...
		/// </summary>
		NoError = 0,
		/// <summary>
		/// Der Dienst ist gerade betriebsbereit
		/// </summary>
		Ready = 1,
		/// <summary>
		/// Startupvorgang wurde abgeschlossen
		/// </summary>
		StartupFinished = 2,
		/// <summary>
		/// Startvorgang konnte nicht abgeschlossen werden
		/// </summary>
		StartupError = 3,
	}
	
	/// <summary>
	/// Auflistung diverser Benutzerbefehle an Services
	/// </summary>
	public enum ServiceCustomCommands
	{
		/// <summary>
		/// An definierten Orten (Festplatte, RAM, ...) liegen neue Daten bereit, die vom Befehlsempfänger eingelesen und verarbeitet werden sollen
		/// </summary>
		NewDataAvailable = 129,
    ...
    /// <summary>
    /// Gibt Auskunft über den Servicestatus einzelner Dienste
    /// </summary>
    public enum ServiceState
    {
        ...
        /// <summary>
        /// Der Dienst, der zu steuern versucht wurde, existiert nicht
        /// </summary>
        NotFound = 0, // The service is not known on the host server.
        /// <summary>
        /// Der Dienst ist gestoppt (NET)
        /// </summary>
        Stop = 1, // The service is NET stopped.
        /// <summary>
        /// Der Dienst ist aktiv / gestartet (NET)
        /// </summary>
	...
	/// <summary>
	/// Die TBL.serviceResponseLog-Klasse erstellt (wenn möglich) einen Serviceexchangelog, über den Nachrichten zwischen Servicecontroller und Services ausgetauscht werden.
	/// </summary>
	public class ServiceResponseLog
	{
		private EventLog log;
		private int communicationID;
		private const string logName = "TBLServiceExchangeLog";
		private string srcName = "";
		
		/// <summary>
		/// Der Konstruktor instanziert einen neuen serviceResponseLog, der unter Umständen auch erst im Betriebssystem angelegt werden muss. Dazu sind Administrationsrechte beim Ausführen nötig!!
		/// </summary>
		/// <param name="pCommunicationID">ID, mit der die Einträge versehen werden, um sie zu identifizieren</param>
		/// <param name="pSourceName">Name der Source der Instanz (use "CONTROLLER") für die ServiceController-Applikation</param>
		/// <remarks>
		/// Der gewählte Sourcename darf nicht in anderen Eventlogs als Sourcename verwendet werden und eine bestimmte Zeichenzahl nicht überschreiten. Empfohlen werden drei Zeichen (zb. svc, cal, ..)
		/// </remarks>
		public ServiceResponseLog(int pCommunicationID, string pSourceName)
		{
			log = new EventLog();
			...
			if(!EventLog.SourceExists(srcName)) // Wenns den Log noch nicht gibt, dann erstelle...
			{
    ...
    /// <summary>
    /// Die ServiceInstaller-Klasse stellt einige Methoden (static) und Funktionen zum Installieren, Überwachen und Steuern von Windows-Diensten zur Verfügung.
    /// </summary>
    /// <remarks>
    /// <para><b>Wichtiger Hinweis:</b>Die Verwendung dieser Klasse wird in den meisten Fällen (v.a. ab Win Vista durch die Benutzerkontensteuerung(UAC)) Administratorprivilegien erfordern. Diese können in den Manifest-Dateien der einzelnen Projekte verlangt werden. Wird das vernachlässigt, kann es u.U. zu schwer zu diagnostizierenden Fehlern kommen.</para>
    /// <para>Es war mir nicht möglich, herauszufinden, wer den Kern dieser Klasse geschrieben hat (wurde als ServiceInstaller im Internet in einem Forum gefunden). Die Klasse wurde erweitert und teilweise modifiziert. Sollten dennoch Ansprüche geltend gemacht werden, bin ich jederzeit bereit, diese Klasse aus der TBL wieder zu entfernen und dem Urheber ggf. nützliche Erweiterungen, die an dieser Klasse vorgenommen wurden, zur Verfügung zu stellen.</para> 
    ///</remarks>
    public class ServiceUtility
    {
        ...
        /// <param name="pStart">Dienst nach erfolgter Installation starten ja/nein</param>
        public static void InstallAndStart(string ServiceName, string DisplayName,
        string FileName, bool pStart)
        {
            IntPtr scman = OpenSCManager(ServiceManagerRights.Connect |
